...
Issue #17/#66/#67 另提供显式选择的 `LocalhostSecureStateSpaceRuntime`。它在三个 `spawn` 角色和父
进程间使用仅限 `127.0.0.1` 的固定 schema TCP，并让 P1/P2 直接交换在线 Protocol 1/2 消息；
父进程每轮只向 Client 发送规范化输入并接收双角色提交后的公开结果。Client 复用同一 Protocol 3
orchestrator，经私有通道调度 P1/P2、重构输出并确认两方提交；它保持
