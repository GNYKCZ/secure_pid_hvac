...
AI 在实现功能时，不得只以“当前 Issue 能通过”为设计目标。
必须同时考虑变化边界、职责归属、依赖方向和未来修改成本。

本节约束的是设计思维，而不是某个具体模块或业务场景。
...
如果一个易变参数变化迫使多个稳定模块同时修改，应视为潜在架构问题，而不是直接继续添加补丁。


### X.2 变化应停留在拥有该变化的层

每个变化都必须有明确 ownership。

某个概念由哪一层拥有，就应尽量只在该层及其直接消费者中传播。
...
> 修改一个概念时，只修改真正拥有该概念的最小区域。

如果同一事实需要在多个地方同时手工修改，优先检查是否缺少单一事实来源。


### X.3 单一事实来源（Single Source of Truth）
...
派生值原则上应由原始事实计算得到，而不是由开发者在多个地方手工重复填写。

如果一个值已经可以从另一个可靠来源确定，默认不再创建第二个独立可编辑副本。


### X.4 依赖具体能力，而不是依赖具体实例
...
而不是依赖某个具体对象实例、文件名、运行 ID、环境名称或业务名称。

优先：

```text
consumer <- interface/contract <- provider

而不是：

consumer -> 某个具体 provider 的内部细节

当调用方只需要“能够提供某种数据/行为”时，不应要求调用方理解提供者的全部实现或身份。

避免把“当前唯一实现”误认为“永久唯一实现”。

X.5 配置、代码、运行状态和产物必须概念分离
...
如果一个文件同时承担多个生命周期完全不同的职责，应检查是否存在不必要耦合。
...
根据目录结构猜测上游状态；
从其他模块内部对象反向获取业务信息；
隐式 singleton / registry。

一个组件如果为了完成简单职责，需要知道整个系统大量上下文，通常说明边界设计不合理。

X.7 控制变化传播（Change Amplification）

实现完成前必须检查一次 change amplification：

假设只改变当前模块拥有的一个合理输入或策略，
需要修改多少个不相关文件或模块？
...
变化范围应与概念 ownership 相匹配。

不得为了降低文件数量而制造巨型模块，也不得为了表面分层而制造跨层同步修改。
...
同时也禁止为了消除少量重复，提前构造复杂抽象体系。

判断标准不是“有没有重复代码”，而是：

这些变化是否属于同一个稳定概念。

X.9 抽象应围绕稳定概念，而不是围绕当前文件结构

不得因为当前有两个相似文件，就自动创建基类、factory、registry 或框架。
...
如果不能回答，不应为了“解耦”而增加抽象。

解耦不等于更多 class/interface/file。
...
这种双向知识通常比 import cycle 更危险，即使代码层面没有循环 import。

依赖方向必须可以用一句话解释。

如果两个模块都需要知道对方大量内部信息，应重新检查职责是否拆错。

X.11 派生信息应自动产生，不应人工同步
...
不要为了验证一致性而制造新的第二事实来源。

X.12 可替换性检查

对于新增或修改的组件，至少问：

如果换掉它的一个合理实现，调用方是否需要理解其内部细节并大量修改？

如果答案是“是”，检查接口是否泄漏了实现细节。

可替换性不要求构造复杂 plugin 系统。

只要求消费者依赖它真正需要的 contract。
...
生命周期差异很大的信息如果被放在一起维护，会产生强耦合。

优先让不同生命周期的信息分别由合适的层管理，再通过明确数据流组合。

X.14 Fail Closed 不等于 Hard Coupling

项目要求严格验证和 fail closed，但不得把“严格”误解为：

把所有具体值硬编码在多个地方；
让每个下游组件知道所有上游内部实现；
人工复制大量 hash / identity；
每个实例创建一套平行代码。
